...
bool combat (struct joueur* j, struct ennemi* e)
{
	...
	int pv_ennemi = e->pv;			
	affiche_texte(1, "Etes-vous sûr de vouloir combattre ? Oui[o] ou Non [n]");
	scanw("%s",&entree); 
	
	if (entree == 'o') 				// On lance le combat si le joueur a rentré 'o'
	{
		while ((j->pv > 0) && (e->pv >0))    // Tant qu'aucun des combattants n'a les pv à 0
		{
			//affiche_texte(0, "Vous : %d | Ennemi : %d   ",j->pv,e->pv);
			affiche_texte(1, "Que voulez vous faire : attaquer[a] ou defendre[d] ? ");
			scanw("%s",&entree);
			
			if (entree == 'a') 
			{
				action = rand()&1;  // Génère un nombre aléatoire entre 0 et 1
				
				if (action == 0) 
				{
					affiche_texte(2, "L'adversaire attaque aussi\n");
					e->pv = e->pv - j->atk;
					j->pv = j->pv - e->atk;
				} 
				else 
				{
					affiche_texte(2, "L'adversaire se défend\n");
					e->pv = e->pv - (j->atk / 2);
				}
			} 
			else if (entree == 'd') 
			{
				...
				{
					affiche_texte(2, "L'adversaire se défend aussi\n");
				}
			}
		}
	} 
	...
	if (e->pv <= 0) 
	{
		...
		//j->pv = pv_max;       		// Remet les pv du joueur a ceux d'avant le combat
